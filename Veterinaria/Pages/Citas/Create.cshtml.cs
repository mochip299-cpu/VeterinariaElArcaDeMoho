...
public class CreateModel : PageModel
{
    ...
    public async Task OnGetAsync(int? mascotaId)
    {
        await CargarSelectListsAsync();
        if (mascotaId.HasValue)
            Cita.MascotaId = mascotaId.Value;
        // Fecha por defecto: próxima hora en punto
        Cita.FechaHoraAtencion = DateTime.Now.AddHours(1).Date
            .AddHours(DateTime.Now.AddHours(1).Hour);
    }
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        await _svc.CrearCitaAsync(Cita);
